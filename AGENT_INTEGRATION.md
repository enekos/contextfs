...
This project uses the Mairu context engine through `mairu-agent` (or compatibility alias `mairu-agent`) to store and retrieve:
- team conventions
...
mairu-agent memory search "test lint file structure conventions" -k 8 -P <project_name>

# Architecture-specific retrieval
mairu-agent memory search "authentication token validation rules" -k 8 -P <project_name>
mairu-agent node search "authentication architecture" -k 8 -P <project_name>
mairu-agent node ls "contextfs://<project_name>/backend/auth" -P <project_name>
```
...
mairu-agent vibe-mutation "Remember: we use Vitest for tests and place integration tests under tests/integration." -P <project_name> -y

# Update known architecture details
mairu-agent vibe-mutation "Update auth context: refresh tokens are now rotated on every renewal and revoked on logout." -P <project_name> -y
```
...
```bash
mairu-agent vibe-query "How does authentication work in this codebase and what are the token validation rules?" -P <project_name>
```
...
mairu-agent memory search "authentication setup" -k 5 -P <project_name> --fuzziness auto --phraseBoost 3 --highlight

# Memory search with strict filtering
mairu-agent memory search "jwt validation middleware" -k 10 -P <project_name> --fuzziness 0 --minScore 5

# Memory write with explicit metadata
mairu-agent memory store "API errors use { code, message, details } shape." -c convention -o agent -i 7 -P <project_name>
```
...
```bash
# Store a node
mairu-agent node store "contextfs://<project_name>/backend/auth" "Auth Module" "JWT access tokens, rotating refresh tokens, and revocation list checks." -P <project_name>

# List a subtree
mairu-agent node ls "contextfs://<project_name>/backend" -P <project_name>
```
...
```bash
mairu-agent node restore "contextfs://<project_name>/backend/auth"
```
...
```bash
mairu-agent daemon . -P <project_name> &
```
...
3. **Scope always:** Include `-P <project_name>` on every `mairu-agent` call.
