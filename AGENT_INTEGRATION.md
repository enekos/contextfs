...
This project uses the Mairu context engine through `mairu-context` (or compatibility alias `mairu-context`) to store and retrieve:
- team conventions
...
mairu-context memory search "test lint file structure conventions" -k 8 -P <project_name>

# Architecture-specific retrieval
mairu-context memory search "authentication token validation rules" -k 8 -P <project_name>
mairu-context node search "authentication architecture" -k 8 -P <project_name>
mairu-context node ls "contextfs://<project_name>/backend/auth" -P <project_name>
```
...
mairu-context vibe-mutation "Remember: we use Vitest for tests and place integration tests under tests/integration." -P <project_name> -y

# Update known architecture details
mairu-context vibe-mutation "Update auth context: refresh tokens are now rotated on every renewal and revoked on logout." -P <project_name> -y
```
...
```bash
mairu-context vibe-query "How does authentication work in this codebase and what are the token validation rules?" -P <project_name>
```
...
mairu-context memory search "authentication setup" -k 5 -P <project_name> --fuzziness auto --phraseBoost 3 --highlight

# Memory search with strict filtering
mairu-context memory search "jwt validation middleware" -k 10 -P <project_name> --fuzziness 0 --minScore 5

# Memory write with explicit metadata
mairu-context memory store "API errors use { code, message, details } shape." -c convention -o agent -i 7 -P <project_name>
```
...
```bash
# Store a node
mairu-context node store "contextfs://<project_name>/backend/auth" "Auth Module" "JWT access tokens, rotating refresh tokens, and revocation list checks." -P <project_name>

# List a subtree
mairu-context node ls "contextfs://<project_name>/backend" -P <project_name>
```
...
```bash
mairu-context node restore "contextfs://<project_name>/backend/auth"
```
...
```bash
mairu-context daemon . -P <project_name> &
```
...
3. **Scope always:** Include `-P <project_name>` on every `mairu-context` call.
