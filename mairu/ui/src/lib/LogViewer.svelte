...
<div class="flex-1 flex flex-col bg-black">
  <div class="h-14 border-b border-green-900 flex items-center px-4 shrink-0 bg-black text-green-500">
    <div class="flex gap-4 h-full">
      <button 
        class="flex items-center gap-2 hover:text-green-300 transition-colors h-14 {activeTab === 'overview' ? 'text-green-400 border-b-2 border-green-400' : 'text-green-600 border-b-2 border-transparent'}"
        on:click={() => activeTab = 'overview'}
      >
        <Activity size={16} /> Overview
      </button>
      <button 
        class="flex items-center gap-2 hover:text-green-300 transition-colors h-14 {activeTab === 'logs' ? 'text-green-400 border-b-2 border-green-400' : 'text-green-600 border-b-2 border-transparent'}"
        on:click={() => activeTab = 'logs'}
      >
        <Terminal size={16} /> Logs
      </button>
      <button 
        class="flex items-center gap-2 hover:text-green-300 transition-colors h-14 {activeTab === 'graph' ? 'text-green-400 border-b-2 border-green-400' : 'text-green-600 border-b-2 border-transparent'}"
    ...
    {#if activeTab === 'overview'}
      <div class="max-w-3xl mx-auto space-y-8">
        <div>
          ...
          <p class="text-green-500">Mairu is connected and ready to navigate your codebase.</p>
        </div>
      ...
      <div class="max-w-3xl mx-auto h-full flex flex-col font-mono text-sm">
        ...
        <div class="flex-1 bg-green-950 border border-green-900 rounded-xl p-4 overflow-y-auto space-y-2">
          ...
          {#if $messages.length === 0 || ($messages.every(m => m.statuses.length === 0 && m.toolCalls.length === 0 && m.logs.length === 0))}
            <div class="text-green-600 text-center py-8 font-sans">No recent activity logs available.</div>
          {/if}
        </div>
      </div>
