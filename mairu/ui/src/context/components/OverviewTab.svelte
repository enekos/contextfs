<script lang="ts">
  ...
  $: catEntries = Object.entries(catCounts as Record<string, number>).sort((a, b) => Number(b[1]) - Number(a[1])) as Array<[string, number]>;
  $: catMax = Math.max(...Object.values(catCounts as Record<string, number>).map((v) => Number(v)), 1);
  ...
  $: impMax = Math.max(...Object.values(impCounts as Record<number, number>).map((v) => Number(v)), 1);

  // Owner distribution
  $: ownerCounts = memories.reduce((acc, m) => {
    const owner = (m.owner as string) || "any";
