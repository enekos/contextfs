...
const centralContextServerUrl = config.centralContextServerUrl.trim();
const centralContextServerToken = config.centralContextServerToken.trim();
...
const centralProxyPaths = new Set([
  "/api/search",
  "/api/memories",
  "/api/skills",
  "/api/context",
...
async function maybeProxyToCentralContext(
  req: IncomingMessage,
  res: ServerResponse<IncomingMessage>,
  parsed: URL
): Promise<boolean> {
  if (!centralContextServerUrl) return false;
  if (!centralProxyPaths.has(parsed.pathname)) return false;

  const upstream = new URL(parsed.pathname + parsed.search, centralContextServerUrl);
  ...
  if (centralContextServerToken) {
    headers["X-Context-Token"] = centralContextServerToken;
  }
...
async function handleRequest(req: IncomingMessage, res: ServerResponse<IncomingMessage>) {
  ...
  try {
    if (await maybeProxyToCentralContext(req, res, parsed)) {
      return;
    }
