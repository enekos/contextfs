...
import { GoDescriber } from "./ast/goDescriber";
...
export class CodebaseDaemon {
  ...
  private readonly goDescriber: GoDescriber;
  ...
  constructor(manager: ContextManager, project: string, watchDir: string, options: DaemonOptions = {}) {
    ...
    this.goDescriber = new GoDescriber();
  }
  ...
  public async initParsers(): Promise<void> {
    ...
    await this.goDescriber.initParsers();
  }
  ...
  public async stop() {
    ...
    this.goDescriber.deleteParsers();
  ...
  private getDescriberForFile(filePath: string) {
    ...
    if (filePath.endsWith(".go")) return this.goDescriber;
  ...
  private summarizeSourceFile(filePath: string, sourceText: string): SourceSummary {
    ...
    return { abstractText, overviewText, nlContent, logicGraphMetadata, symbols: result.symbols };
  }
  ...
  private async processFile(filePath: string) {
    ...
    // Write individual symbols for surgical reads by Mairu agent
    const mairuSymbols = summary.symbols.map(s => ({
      id: Buffer.from(`${uri}::${s.id}`).toString('base64url'),
      file_path: absPath,
      name: s.name,
      kind: s.kind,
      exported: s.exported,
      start_row: s.startLine,
      end_row: s.endLine,
      start_col: 0,
      end_col: 0,
    }));
    // Older test doubles may not implement symbol upserts yet.
    if (typeof (this.manager as any).upsertSymbols === "function") {
      await (this.manager as any).upsertSymbols(absPath, mairuSymbols);
    }
